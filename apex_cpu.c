...
print_reg_file(const APEX_CPU *cpu)
{
    ...
    for (int i = 0; i <= 15; ++i)
    {
        printf("M%-3d[%-3d] ", i, cpu->data_memory[i]);
    }
...
APEX_decode(APEX_CPU *cpu)
{
    if (cpu->decode.has_insn)
    {
        ...
        case OPCODE_LDR:
        {
            if ((!regbuffer[cpu->decode.rs1] && !regbuffer[cpu->decode.rs2]))
            {
                cpu->decode.rs1_value = cpu->regs[cpu->decode.rs1];
                cpu->decode.rs2_value = cpu->regs[cpu->decode.rs2];
                regbuffer[cpu->decode.rd] = 1;
        ...
        case OPCODE_STORE:
        {
            if (!regbuffer[cpu->decode.rs1] && !regbuffer[cpu->decode.rs2] && !regbuffer[cpu->decode.rd])
            {
                cpu->decode.rs1_value = cpu->regs[cpu->decode.rs1];
                cpu->decode.rs2_value = cpu->regs[cpu->decode.rs2];
                cpu->decode.stalled = 0;
...
APEX_memory(APEX_CPU *cpu)
{
    if (cpu->memory.has_insn)
    {
        ...
        {
            /* Read from data memory */
            cpu->data_memory[cpu->memory.memory_address] = cpu->memory.rd_value;
            break;
        }
        }
